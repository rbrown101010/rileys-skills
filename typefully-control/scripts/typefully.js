...
const API_ROOT = 'https://api.typefully.com';
const API_BASE = process.env.TYPEFULLY_API_BASE || `${API_ROOT}/v2`;
...
async function apiRequest(method, endpoint, body = null, opts = {}) {
  const result = await apiRequestDetailed(method, endpoint, body, opts);
...
async function apiRequestDetailed(method, endpoint, body = null, opts = {}) {
...
async function cmdApiProbe() {
  const checks = [];

  for (const endpoint of ['/v3/me', '/v2/me']) {
    const result = await apiRequestDetailed('GET', endpoint, null, { exitOnError: false });
    checks.push({
      endpoint,
      ok: result.ok,
      status: result.status,
      url: result.url,
      keys: result.data && typeof result.data === 'object' && !Array.isArray(result.data)
        ? Object.keys(result.data).slice(0, 10)
        : [],
    });
  }

  const preferredPublicApiBase = checks.find(check => check.ok)?.url.replace(/\/me$/, '') || null;

  output({
    checked_at: new Date().toISOString(),
    configured_api_base: API_BASE,
    preferred_public_api_base: preferredPublicApiBase,
    checks,
  });
}

async function cmdApiRequest(args) {
  const parsed = parseArgs(args, { 'include-headers': 'boolean' });
  const method = (parsed._positional[0] || '').toUpperCase();
  const endpoint = parsed._positional[1];
  ...
  if (parsed.body && parsed['body-file']) {
  ...
  if (parsed['body-file']) {
    if (!fs.existsSync(parsed['body-file'])) {
      error(`File not found: ${parsed['body-file']}`);
    }
    const raw = fs.readFileSync(parsed['body-file'], 'utf-8');
    try {
      body = raw ? JSON.parse(raw) : null;
    } catch {
      error('--body-file must contain valid JSON');
    }
  } else if (Object.prototype.hasOwnProperty.call(parsed, 'body')) {
    try {
  ...
  const result = await apiRequestDetailed(method, endpoint, body, { exitOnError: false });
  const payload = {
    ok: result.ok,
    status: result.status,
    url: result.url,
...
SETUP:
  ...
  api:probe                                  Check whether the live public API exposes v3 or v2
  api:request <method> <endpoint> [options]  Make a raw authenticated request to Typefully
    --body <json>                            JSON request body
    --body-file <path>                       Read JSON request body from file
...
EXAMPLES:
  ...
  # Probe which public API version is live
  ./typefully.js api:probe

  # Make a raw request against a versioned endpoint
  ./typefully.js api:request GET /v2/social-sets

  # Make a raw request using the configured base URL
  ./typefully.js api:request GET /social-sets
...
const COMMANDS = {
  ...
  'api:probe': cmdApiProbe,
  'api:request': cmdApiRequest,
