...
description: Create, repair, validate, preview, and package Codex-compatible animated character spritesheets from character art, screenshots, generated images, or visual references. Use when a user wants to turn a reference image or concept into an animated sprite character, hatch a Codex pet, create a custom animated pet, or build an 8x9 atlas with transparent unused cells, row-by-row animation prompts, QA contact sheets, preview videos, and pet.json packaging. This skill composes the installed $imagegen system skill for visual generation and uses bundled scripts for deterministic spritesheet assembly.
---
...
Create a Codex-compatible animated sprite character from a concept, one or more reference images, or both. The current packaged output is the Codex pet format, so this skill keeps the proven atlas, QA, preview, and `pet.json` pipeline while presenting it as a more general reference-image-to-animated-sprite workflow. It delegates visual generation to `$imagegen`.
...
```bash
SKILL_DIR="${CODEX_HOME:-$HOME/.codex}/skills/sprite-animation"
...
```text
Generate the `<row-id>` row for this sprite-animation run.
